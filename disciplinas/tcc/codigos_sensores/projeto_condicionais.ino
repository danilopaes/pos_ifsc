...
// ===== FUNÇÕES =====
// Função para adicionar uma nova entrada JSON ao log no arquivo aberto
// file: ponteiro para o arquivo aberto no cartão SD
// jsonEntry: string JSON contendo os dados a serem adicionados
// Retorna true se a gravação foi bem-sucedida, false caso contrário
bool appendJsonLog(File &file, const String &jsonEntry) {
  if (!file) {
    Serial.println("Arquivo não está aberto.");
    return false;
  }
  
  // Move o cursor para o final do arquivo para adicionar a nova entrada
  file.seek(file.size());
  
  // Adiciona a entrada JSON seguida de uma quebra de linha para facilitar leitura
  if (file.println(jsonEntry)) {
    file.flush(); // Garante que os dados sejam gravados no cartão SD
    return true;
  } else {
...
void setup() {
  ...
  // SD Card
  if (!SD.begin(10)) { // Ajuste o pino CS conforme seu módulo SD
    Serial.println("Falha ao inicializar o cartão SD");
    return;
  }

  // Abre o arquivo em modo append (adicionar ao final)
  logFile = SD.open("log.json", FILE_WRITE);
  if (!logFile) {
...
void loop() {
  ...
  // Chama a função para adicionar a entrada ao arquivo
  snprintf(msg, sizeof(msg), "%.2f,%.2f,%d,%.2f,%.2f,%d,%.2f", temp, hum, gas_mq2, gas_mq135, dist, soil_hum, temp_soil);
  if (appendJsonLog(logFile, msg)) {
      Serial.println("Entrada gravada com sucesso.");
  } else {
      Serial.println("Falha ao gravar a entrada.");
  }
